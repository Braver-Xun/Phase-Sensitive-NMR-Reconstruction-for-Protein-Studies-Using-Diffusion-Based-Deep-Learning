...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_in, noise=None):
        ...
        # x_noisy = (x_noisy + 0.02*x_in['LR'])/(1 + 0.02)
        if not self.conditional:
            x_recon = self.denoise_fn(x_noisy, continuous_sqrt_alpha_cumprod)
        else:
            x_recon = self.denoise_fn(
                torch.cat([x_in['SR'], x_noisy], dim=1), continuous_sqrt_alpha_cumprod)
        noise_decrease_one_dim = noise * x_in['Mask']
        x_recon_decrease_one_dim = x_recon * x_in['Mask']
        loss = 1*self.loss_func(x_recon_decrease_one_dim, noise_decrease_one_dim) + 0.1*self.loss_func(x_recon, noise)
        return loss
